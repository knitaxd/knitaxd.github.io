...
const sneakerIdentify = document
    .querySelector("#main__info--button")
...
function loadEventListeners() {
    ...
    addCartButton.addEventListener("click", cartQuantity);

    // Delete sneakers in the cart
    cart.addEventListener("click", deleteSneaker);
}

// Count the over items in the cart
function cartQuantity() {
    let overNumber = document.querySelector("#number__items");
    totalSneakers = totalSneakers + parseInt(quantityItem.value);
    overNumber.textContent = `${totalSneakers}`;
}
...
function addSneaker() {
    ...
    if (exist) {
        ...
        const sneakers = cartItems.map((sneaker) => {
            if (sneaker.id === sneakerInfo.id) {
                ...
                sneaker.finalPrice =
                    parseInt(sneakerPrice.textContent.substring(1)) * sneaker.quantity; // Calculate the final price
    ...
    function sneakerTotal() {
        return (totalPrice =
            parseInt(sneakerPrice.textContent.substring(1)) * quantityItem.value);
    }
}
...
function deleteSneaker(e) {
    ...
    if (e.target.classList.contains("delete__item--button")) {
        ...
        // Refreg th over sneakers quantity
        overNumber.textContent = 0;
        totalSneakers = 0;
